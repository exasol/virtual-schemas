 ...
 * <p>
 * Dialects define the capabilities and the behavior of the adapter
 * implementation for a specific remote database.
 * <p>
 * For information about how the metadata of the remote data source is mapped,
 * please refer to the list of interfaces below.
 *
...
public interface SqlDialect {
    ...
    /**
     * Multiplicity that the remote data source supports for a structural element
     * like catalogs or a schemas.
     * <p>
     * <dl>
     ...
     * </dl>
     * </p>
     * Dialects that support a single database should not use
     * <code>AUTO_DETECT</code> because this unnecessarily costs performance.
     */
    ...
    /**
     * Define whether the remote source supports catalogs and if it does, whether is
     * supports a single "pseudo" catalog or multiple catalogs.
     *
     * @return <code>NONE</code> if the dialect does not support catalogs at all,
     *         <code>SINGLE</code> if it supports exactly one (like e.g. Exasol) or
     *         <code>MULTIPLE</code> if real catalogs are supported.
     */
    public StructureElementSupport supportsJdbcCatalogs();

    /**
     * Define whether the remote source supports schemas and if it does, whether is
     * supports a single "pseudo" schema or multiple schemas.
     *
     * @return <code>NONE</code> if the dialect does not support schemas at all,
     *         <code>SINGLE</code> if it supports exactly one (like e.g. Exasol) or
     *         <code>MULTIPLE</code> if real schemas are supported.
     */
    ...
    /**
     * Check whether the dialect expects table identifiers to be qualified with the
     * catalog
     *
     * <p>
     * Example:
     ...
     *         the table name. In many cases this is a dot.
     */
     ...
     * <dd>Independently of the actual search order <code>NULL</code> values always
     * appear at the <em>end</em> of the result set</dd>
     * <dt><code>NULLS_SORTED_AT_START</code></dt>
     * <dd>Independently of the actual search order <code>NULL</code> values always
     * appear at the <em>start</em> of the result set
     * <dt><code>NULLS_SORTED_HIGH</code></dt>
     * <dd><code>NULL</code> values appear at the start of the result set when
     * sorted in descending order, and at the end when sorted ascending</dd>
     * <dt><code>NULLS_SORTED_LOW</code></dt>
     * <dd><code>NULL</code>values appear at the end of the result set when sorted
     * in descending order, and at the start when sorted ascending</dd>
     */
    ...
    /**
     * Rewrite the given query so that data from the remote data source is imported
     * into Exasol when that query is executed on the Virtual Schema frontend.
     *
     * @param statement   original query as sent by the Virtual Schema frontend
     * @param exaMetadata Exasol metadata
     * @return rewritten query
     * @throws SQLException     if execution of any SQL command on the remote data
     *                          source failed
